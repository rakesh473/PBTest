...
namespace PBSqlite.Services
{
    ...
    public class RazorRenderService : IRazorRenderService
    {
        ...
        {
            var getPageResult = _razorViewEngine.GetPage(null, pageName);
            if (getPageResult.Page != null) return getPageResult.Page;
            var findPageResult = _razorViewEngine.FindPage(actionContext, pageName);
            if (findPageResult.Page != null) return findPageResult.Page;
            var searchedLocations = getPageResult.SearchedLocations.Concat(findPageResult.SearchedLocations);
            var errorMessage = string.Join(
                Environment.NewLine,
                new[] { $"Unable to find page '{pageName}'. The following locations were searched:" }.Concat(
                    searchedLocations));
